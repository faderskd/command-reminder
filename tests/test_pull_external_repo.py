...
class PullExternalRepositoryTestCase(BaseTestCase):
    ...
    def test_should_generate_init_script_importing_external_main_and_external_repositories(self):
        # given
        parser.parse_args(['init'])
        parser.parse_args(['pull', '--repo', 'https://github.com/faderskd/common-commands'])
        ...
        with assert_stdout() as stdout:
            self.assertOutputContains(stdout.output,
                                      f'set -gx {FISH_FUNCTIONS_PATH_ENV} \
                                      ${FISH_FUNCTIONS_PATH_ENV} {TEST_TMP_DIR_PATH}/{REPOSITORIES_DIR_NAME}/{MAIN_REPOSITORY_DIR_NAME}/{FISH_FUNCTIONS_DIR_NAME} \
                                      {TEST_TMP_DIR_PATH}/{REPOSITORIES_DIR_NAME}/faderskd_common_commands/{FISH_FUNCTIONS_DIR_NAME}')
